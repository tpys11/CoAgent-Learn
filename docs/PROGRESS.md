...
| **F14** | **AI 服务整合轮**（三层模式首轮：任务书→设计会话→执行） | 设计中 | **🔄 owner 已启动设计会话（09-01）**——范围（①设置页信息架构重组：按用途三组 ②配置自检卡 ③OpenCode Zen 供应商接入（OpenAI 兼容 /v1，角色级模型 MAIN/REVIEW 分离）④预设档位：标准/免费体验/自定义 ⑤T51 顺手修；**默认档保持 DeepSeek 不变**；真网冒烟含免费档全链一次） |
...
| 37 | **三层派发**（owner 09-01 立项，同日修订裁撤审计） | 总领任务书（可行性+模块拆分，不含子步）→ 设计会话（深度侦察+详细设计+工程化控制，不执行→ `docs/dispatch/step-<id>-design.md`）→ **owner 直发执行（总领设计审计已裁撤）** → 执行会话对照设计执行（结构性偏差上报总领，微偏差自裁）→ 总领验收。适用判据：多文件/跨层/有架构取舍的轮次走三层；总领已深度侦察的轮次可豁免设计层直发（F13 先例）。**补偿控制**：执行复述门对照设计复核锚点 + 总领验收不变。首例=F14（设计中） | owner 裁撤审计环节（09-01）；F10 复述门两修正证明执行层可承接前提纠偏 |
