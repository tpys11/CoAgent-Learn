...
| **F14** | **AI 服务整合轮**（三层模式首轮） | 执行🟡 | **S1-S5c 代码落地、总领验收中**（18 commit `0973fa0..e069027` 对照设计 §2；总领实证：f14 后端 24 + vitest 205 + tsc 0 双条件；变异②exclude_unset 恰 1 红=F8 E4 复现钉、变异①存活定性纵深防御；**CI e069027 failure→根因=probe 测试宿主 .env 依赖→总领修 `2915e81`**；真实库 10946 未动；dev 栈对齐 `BuMoXeg2`；执行层流程泄漏 5 项→**E-39**；详情 `docs/progress/step-F14.md` 总领补记版）。**S6 E2E ⏳ 待 owner 在 f14tmp UI 填 Zen key**（A4/A5 活体） |
