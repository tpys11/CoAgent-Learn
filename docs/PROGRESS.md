...
| **T50** | **🔴【数据丢失事件 · P0】「ai Agent」课程知识库向量在 F7 会话窗口内消失**：总领 02:13 查询该库 3 条真实向量结果 → F7 会话后同查询空，只读直查 `kb_vectors` 该项目 **0 行**（现库仅存 smoke-web1 10136 + smoke-web3 371 行，**两项目无 projects 表行 = 孤儿向量**）；`kb_tree` 残留孤儿（ai_agent 8 + smoke 3）。时间括号铁证：数据在 F7 窗口内消失。**疑似触发**：smoke-web1（10136 块）写入/重建路径跨项目清除——**待 F7 会话交代完整操作序列**。恢复：owner 重传源 PDF 即可重建 | 总领只读直查取证（mode=ro）+ 复查补充证据 | **F7 会话序列已交代（step-F7.md §T50）**：零写触达真实库（唯 pytest，实证非破坏）；smoke-web* 无 tracked 写入者；删除足迹=delete_kb_project 签名（kb_tree 残留吻合）；删除早于 F7 会话首笔 DB 读——**余下待 owner 调取前序会话记录**；F10 动工前必读 |
