...
| **R-A5** | **结构收敛+冒烟第三批修复**（owner 授权重构：审核路由三处复刻收敛为 resolve_review_route 单一事实源+GET effective_model；Zen key 专用保存通道；气泡上下排列；开关 ON 补写 research 兑现气泡承诺。**E-40 背景**：反馈①③主因是后端容器陈旧，已当场重建） | 提示词已交付 | **🔄 docs/dispatch/step-RA5.md（09-01，重构直发）**——S1 后端单一事实源/S2 saveZenKey 专用通道/S3 前端消费收敛+布局/开关 research 补写/S4 变异×2；验收含容器形态核对 |
