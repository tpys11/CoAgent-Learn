...
| **WAVE2 终判** | 全量 54 例三指标终数（judge 统一重判 + 独立复算双列） | 2026-09-05 | **✅ 终数落定（正式提交口径）**：**幻觉 虚构 0/claims 744 ✓**（无支撑 42=检索缺口 34+无引用 8；L2 异厂复判 54/54 False；陷阱 0/18）｜**适配 48/50=96.0% ✓**（P1 二轮 16/18=88.9%，塌陷版 2/17 前后对照；P2 17/17、P3 15/15 均 100%；校准 n=50 偏差 0.089/绝对 0.117）｜**覆盖 76/112=67.9% ✗**（P1 篇幅减半条款的字面命中张力 71.1%→63.2% 如实披露；36 条未命中全进 L3 判定桶）；L1 引用无效 139/375=37.1%（逐例加总；judge 聚合 375/375 为已知缺陷§5）；审核门 41 打回/25 例；valid_ratio 52/54=0.963。产物：wave2-report.md v2（11 项披露）+ 雷达图 report.html 刷新 + dataset_examples 两组；manual-review 复核单取消（owner 裁定直接采信判卷终值，不再走独立复核单流程；P3-13 口径错配实证移入报告披露⑦） |
