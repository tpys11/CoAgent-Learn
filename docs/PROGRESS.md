...
| **R-A3** | **冒烟反馈第二轮**（owner 2 条：①主通道行仍显 Qwen2.5-72B=LS.model 历史杂值回显，且 pipeline_v2:170 证实发送路径真认该值=显示+发送双治 ②测试档 key 保存反馈弱+立即检测未按档位选探测源=前端 SelfCheckCard:77 恒吃 chat 键） | 提示词已交付 | **🔄 `docs/dispatch/step-RA3.md`（09-01，微轮直发，零后端）**——S1 resolveChatModel 纯函数（标准档钉死 dsv4f 无视杂值/zen 取 LS.model，显示+fetch body 同源）/S2 自检档位感知（chat_zen 源条件化）+保存反馈强化/S3 变异×2 |
