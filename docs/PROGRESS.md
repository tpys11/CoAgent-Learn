...
| **EVAL-1 Wave 2** | 解锁提示词 2026-08-31 交付 owner（F13 不阻塞：KB 内容改 evaltmp 栈 API 上传） | 待 owner 粘贴 | **⏳ 解锁词已交付**（跑 60 用例 → 三指标实测 → 测试数据包组装 → 脚本终稿；副本栈 -p evaltmp + E-37 双 -f 铁律 + MinerU 配额护栏）；`tests/eval/` 骨架已出现在工作区（untracked，授权产物） |
...
1. 10 分钟演示视频（脚本→录制：差异化画像输入→协同调度可视化→资源生成闭环）——按 F11 完成后形态录；前置 N4 ✅（08-31 已闭，dev 前端已载入新 bundle）；**F12 收口后 bundle 需再对齐一次终态**
