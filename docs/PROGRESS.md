...
| **N4** | `docs/dispatch/step-N4.md`（2026-08-31） | 待派发 | **⏳ 提示词已交付**（E-37 遗留收口：override 补 frontend 本地构建并重建 dev 前端容器——F8/F11 前端功能上 dev 栈，9-5 录视频前置；零产品代码零 commit） |
...
| **E-37** | **【环境 · 总领失误认领 08-31】E-36 修复时违反 E-34 单 `-f` 操作，把开发栈打回定版镜像代码**：override 在 `deploy/` 下不在项目根 → 未被自动加载 → compose 按配置漂移把 backend 按纯镜像重建（F8/F11 功能在 dev 下线，S5 端点 404）；E-36 时 `/app/skills` 检查为假阳性（镜像自带）。**已修复**：双 `-f up -d` 恢复代码挂载（S5 冒烟 200）。**遗留：前端容器仍是纯镜像 bundle → 已立 N4 收口**。教训：本地栈必走双 `-f`；挂载验证不得用镜像自带内容当证据 | 总领现场定位（inspect 挂载对比）+ 认领 |
...
1. 10 分钟演示视频（脚本→录制：差异化画像输入→协同调度可视化→资源生成闭环）——按 F11 完成后形态录；**前置：N4 前端 bundle 重建**
