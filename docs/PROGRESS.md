...
| **R-A2** | **R-A 冒烟反馈修复**（owner 4 条：①主模型行应显 dsv4f ②审核行显具体名+短语改名 ③Zen key 输入清空=前端 :136 主动清空（后端无病）④气泡缺模型名+bge 清算+独立审核气泡） | **🔄 打回一条** | **3 commit `a20c159..5ab71bf`；CI success=534/255/0 亲核；三红线零触碰；气泡双联+小字/bge 清算全过（wikiEntries 实路径 src/data/）；需求①③④闭环 ✓；需求②部分——model 字段已具体化但 owner 点名短语 `text:'研究档判卷=主模型同源'`（selfCheck.ts:76）仍在→补充提示词已交付原会话**。**总领认领：派发单根因①误判**（SelfCheckCard.tsx:38 实证早直读 LS.model，RA-S4 起）；「千问」目击候补=浏览器 LS.model 历史值/非 follow_main 审核行诚实显示旧值，复验分流路径在交接 §7 |
