...
| **E-46** | **【外部依赖行为变更 · 全量跑数排障】Console Go 上游收紧空 user content 校验**：00:42 Run 2 学情评估尚正常（level 0.3/0.25），06:53 起同代码 400 "message content cannot be empty"——网关侧变更非我方回归；表现=assess 重试 3 败→兜底 level_score:null→适配列全 None。**教训：凡 chat 调用 user 消息必须非空**（对话生成天然合规故未暴露）；全量 54 例 P1 批 6/18 中途叫停止损 | owner 直令"修"→FIXASSESS（b72c164，673 全绿） | 全弧：修复→三批全量 54 例重跑完成（并行）→判卷（断言幻觉 0 ✓/陷阱 0/18 ✓/适配 34/49=69.4% ✗/覆盖 79/112=70.5% ✗）→P1 适配 2/17 崩塌取证→存量缺陷 prev_score 只读缓存→FIXPIPE（a8a1204）→**P1 批二轮重跑待 owner**（P2/P3 fit 100% 不动；复算列=独立复算权威，judge 聚合三缺陷已知） |
