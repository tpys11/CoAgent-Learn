...
| **F12** | `docs/dispatch/step-F12.md`（2026-08-31） | 进行中 | **🔄 owner 已派发，进行中**：4 commit `332a0b5..2ef67c0` 已落地 S0（T53 删项目介绍，边界判定=Agent 设定持久化与 AgentsView 共用故保留）/S1（T54 删切块栏+T48 死链与 D2-D5+T56 公式定界）/S2（记忆单框化+旧数据兼容层）/S3（三处统一 MemoryBox）；S4 压缩摘要对接+S5 E2E 待收；收口后总领验收 |
| **EVAL-1 Wave 2** | 解锁提示词 2026-08-31 交付 owner | **🟡 工具链就绪、跑数挂起**（**owner 停令：系统未完成前零 API 花费**——停令执行经总领亲证：B1 起 18 例零启动 / evaltmp 栈与副本库零残留 / 真实库 kb_vectors 10946 行未动）。**已交付**：tests/eval 工具链 5 件（60 用例集/runner/judge/stackprep/kb_manifest）smoke 实证；视频脚本 v1.1 终稿（占位全替换，仅镜 10 指标数字待回填）；kb-slice 清单定稿（9 文档/1501 块实测/评委复现指引，MinerU 零调用）。**复跑**：4 命令（stackprep→起栈→runner→judge+汇总）约 3-4h，工具链无需重做；触发信号=owner 宣布系统完成（F12+F9+F10+F13 收口） |
...
| F9 | 知识库结构化 | 待派发 | 队列（D-01=A 五轮全跑） |
| F10 | 上传优先初始化 | 待派发 | 队列（F9 后；**前置 T50 结论**） |
...
| T49 | **【测试基建】宿主 pytest 与开发栈共用 `data/app.db`**（conftest 无 DB 隔离，`:42` 触达 `db_path`）——测试写操作直达真实库，364 测试 × 16 轮未出事故属侥幸面 | F7 会话上报，总领核验属实 | 待专项核实与隔离方案（勿与 T50 混同） |
| **T50** | **🔴【数据丢失事件 · P0】「ai Agent」课程知识库向量在 F7 会话窗口内消失**：总领 02:13 查询该库 3 条真实向量结果 → F7 会话后同查询空，只读直查 `kb_vectors` 该项目 **0 行**（现库仅存 smoke-web1 10136 + smoke-web3 371 行，**两项目无 projects 表行 = 孤儿向量**）；`kb_tree` 残留孤儿（ai_agent 8 + smoke 3）。时间括号铁证：数据在 F7 窗口内消失。**疑似触发**：smoke-web1（10136 块）写入/重建路径跨项目清除——**待 F7 会话交代完整操作序列**。恢复：owner 重传源 PDF 即可重建 | 总领只读直查取证（mode=ro）+ 复查补充证据 | F7 质询已交 owner 转达，**回音未至**；F10（上传链路）动工前必须拿到结论 |
...
1. 10 分钟演示视频——**脚本 v1.1 终稿 ✅**（占位全按落地形态替换：思维链六段/子代理五要素/审核在链内，P1-01 smoke 实拍锚点已写入；仅镜 10 指标数字待跑数回填）；录制前置：F12 收口后 bundle 终态对齐 + owner 录制
