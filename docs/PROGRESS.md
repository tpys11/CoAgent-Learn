...
| **E-46** | **【外部依赖行为变更 · 全量跑数排障】Console Go 上游收紧空 user content 校验**：00:42 Run 2 学情评估尚正常（level 0.3/0.25），06:53 起同代码 400 "message content cannot be empty"——网关侧变更非我方回归；表现=assess 重试 3 败→兜底 level_score:null→适配列全 None。**教训：凡 chat 调用 user 消息必须非空**（对话生成天然合规故未暴露）；全量 54 例 P1 批 6/18 中途叫停止损 | owner 直令"修"→FIXASSESS（b72c164，673 全绿） | 全量 P1-P3 修复后重跑（原 6 例仅作旁证不作官方口径） |
