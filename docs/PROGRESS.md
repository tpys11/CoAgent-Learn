...
| **F14** | **AI 服务整合轮**（三层模式首轮：任务书→设计会话→执行） | 待设计 | **🔄 任务书已交付 owner（08-31 旧执行提示词作废）**——范围（①设置页信息架构重组：按用途三组 ②配置自检卡 ③OpenCode Zen 供应商接入（OpenAI 兼容 /v1，角色级模型 MAIN/REVIEW 分离）④预设档位：标准/免费体验/自定义 ⑤T51 顺手修；**默认档保持 DeepSeek 不变**；真网冒烟含免费档全链一次） |
