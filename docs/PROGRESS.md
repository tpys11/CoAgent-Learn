...
| **F9** | 知识库结构化（`docs/dispatch/step-F9.md`，提示词 2026-08-31 交付 owner） | 待派发 | **⏳ 提示词已交付**（书签/标题提取 → 正文/其他双通道切割+留存范围 UI → kb_tree 层级化（迁移+反向脚本）→ 三处大纲统一渲染 → 真实教材 E2E；**kb_tree 跨项目删除防护守卫=硬约束（T50 防御）**） |
| F10 | 上传优先·平滑初始化 | 待派发 | 队列（F9 后）；T50 处置已定：F7 关闭归档（owner 08-31 回 1），**防御性条款折入派发单**（kb_tree/上传路径跨项目删除防护+守卫），归因由总领挖 F7 会话记录补充 |
...
| **T50** | **🔴【数据丢失事件 · P0】「ai Agent」课程知识库向量消失**：总领 02:13 查询 3 条真实向量结果→事后 0 行；smoke-web1/web3 成孤儿向量（10136+371，无 projects 行）；kb_tree 残留孤儿。**已破案到签名级（08-31）**：①**F7 自证清白**（判定实验：d3687a0 旧套件打真实形态库 ro 备份→前后行数全等；序列交代唯写触达=05:54 pytest）；②删除签名=`delete_kb_project`（向量/问题/边删、**kb_tree 不级联**）；③**总领实测修正 F7 一处**：projects 行**未幸存**（live 查询 projects=2 行、kb_tree=39 pids 其中 37 孤儿）→ 签名匹配**完整课程删除**，且 kb_tree 不级联正是 37 个孤儿 pids 的沉积根因（T49 直打真实库惯例的历史沉积）；④**总领认领**：原「F7 窗口内消失」措辞不成立——删除发生于 02:13–05:45，**早于 F7 会话首笔 DB 读**；⑤取证损失：02:50–05:45 段 uvicorn 访问日志随 06:5x 容器重建销毁（F7 披露，E-38）。**下一步**：owner 回忆/确认 8-31 凌晨是否删过课程 + 总领挖前序会话记录；**防御**：F9 派发单已加 kb_tree 跨项目防护+级联一致性守卫；恢复=owner 重传源 PDF | 总领只读取证 + F7 交代（step-F7.md §T50）+ 双向修正 |
