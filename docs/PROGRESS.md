...
| **F10** | 上传优先·平滑初始化（`docs/dispatch/step-F10.md`，提示词 2026-08-31 交付 owner） | 待派发 | **⏳ 提示词已交付**（上传与向导并行 → 切割完成打断向导插「知识库处理选择」（复用 F9 留存面板/apply-scope）→ 孤儿级联收尾（kg_edges/kb_gen_questions，F9 §6.3 移交）→ 真实教材 E2E） |
