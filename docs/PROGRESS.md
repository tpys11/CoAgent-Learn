...
| **N5** | ~~compose 重钉+README+pull 复验~~ **前半=拉取路径探针 ✅（09-02 总领亲测）**：从零 clone(9.7MB)→匿名 GHCR pull→无 override/无 .env 纯 compose up→双容器 healthy→healthz/前端 200→评委首屏空态正确（E-26 种子在）——**CANTOPEN 风险排除，交付出无病**。**后半（终钉+README+T52 提示语）待体验轮收口后执行**（终钉时机=最后一轮代码后，防白钉） | 前半 ✅ / 后半待派发 | 预期体验轮收口后一次完成；探针发现：container_name 硬编码致本机探针须停 dev 栈保真跑（评委机无此问题，E-35③ 同类已备案） |
...
| **GO** | **测试档第二通道 go**（owner 09-04 直令总领直改：独立网关 URL+Key、主 GLM-5.3-Flash/审核 Qwen3.8 Flash、zen/go 开关互斥——开 A 自动关 B、关 A 不动 B、全关=标准档） | **✅ 已完成（4 commit S1-S4）** | S1 REGISTRY go 档定值格+TEST_CHANNEL 定向+detect_tier 精确相等判定+pick_judge go 分支（8edc397；14 守卫+矩阵守卫 2→3 档更新，变异亲放恰 1 红）/S2 settings GET/PUT go 节+test_channel 白名单+chat_go 探测（6cbb635）/S3 前端镜像 go 常量+三参化+LS.goBaseUrl+saveGoKey 专用通道（2c87019）/S4 测试档卡双通道互斥 UI+go 卡+selfCheck go 分支（db57be8）；tsc 0+vitest 322+pytest 616。**API ID 字面占位待 owner 提供确切值**（决策 40，401 时三处字面各改一行）；**dev 栈未重建**——复验前须双 -f up -d --build（E-40 守则） |
