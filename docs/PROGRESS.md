...
| **F10** | 上传优先·平滑初始化（`docs/dispatch/step-F10.md`，提示词 2026-08-31 交付 owner） | 进行中 | **🔄 复述门已过（08-31 总领照准两修正）**：修正 A=S3 改**守卫形态**（F9 §6.3 系误报——delete_kb_project :398/:400 与 by-source :379/:382 早含两表清理，总领实证；真实缺口=守卫缺口，产品代码零改动）；修正 B=打断层挂 **dialogue 向导**（project 模式死分支，6797378 已移除入口）。S1 事件桥 kbScopeBus（修「弹窗关闭→轮询 setState 丢弃→留存面板永不再现」缺口）开工 |
