...
import { MODEL_MAIN as DEFAULT_MAIN_MODEL, MODEL_ZEN_REVIEW, MODEL_GO_REVIEW, MODEL_ZAI_REVIEW } from '../../models'   // R-D S5：缺省主模型名入注册表镜像（删除本地字面量）
export interface SelfCheckInput {
  ...
  zaiKeySet?: boolean                     // C1：后端 ZAI_API_KEY（GET zai.api_key_set）
...
export function computeSelfCheckRows(i: SelfCheckInput): SelfCheckRow[] {
  ...
  // goKeySet（S4）；glm-4.7=zai 通道看 zaiKeySet（C1）；其余（Qwen2.5-72B 等）=SF 通道看 embeddingKeySet。
  ...
  const isZaiReview = effModel === MODEL_ZAI_REVIEW
  const reviewKeyOk = isZenReview ? i.zenKeySet : (isGoReview ? i.goKeySet : (isZaiReview ? i.zaiKeySet : i.embeddingKeySet))
  rows.push({
    id: 'review',
    state: reviewKeyOk ? 'ok' : 'warn',
    text: reviewKeyOk ? '审核模型已配置'
      : (isZenReview ? '审核模型需要 Zen Key' : (isGoReview ? '审核模型需要 GO Key' : (isZaiReview ? '审核模型需要 Z.AI Key' : '审核模型需要硅基流动 Key'))),
