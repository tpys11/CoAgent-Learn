...
import { X, Settings, Square, Upload, Folder } from 'lucide-react'
...
export default function AgentSettingsModal({ agents, onSave, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50"
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl h-[80vh] flex mx-4" onMouseDown={e => e.stopPropagation()}>
        {/* 左侧 Agent 列表 */}
        <div className="w-48 bg-[#f5f5f5] rounded-l-2xl border-r border-[#e5e5e5] flex flex-col flex-shrink-0">
          ...
          <div className="p-2 border-t border-[#e5e5e5]">
            <button onClick={() => { handleSave(); onClose() }} className="w-full py-2 bg-[#1a1a1a] text-white text-xs font-semibold rounded-lg hover:bg-[#333]">保存</button>
          </div>
        </div>
