  ...
  const renderBtn = (key: ViewKey, Icon: any, label: string, active: boolean) => (
    expanded ? (
      <button
        ...
        title={label}
        className={`w-3/4 ml-auto h-9 mb-2 flex items-center gap-3 px-3 rounded-md text-sm font-medium transition-colors ${
      ...
      {expanded ? (
        <div className="flex flex-col gap-1.5">
          <button onClick={() => onChange('tutorial')} title="使用引导"
            className={`w-3/4 ml-auto h-9 flex items-center gap-3 px-3 rounded-md text-sm font-medium transition-colors ${view === 'tutorial' ? 'bg-[#1a1a1a] text-white' : 'text-dim hover:bg-[var(--bg-hover)] hover:text-[var(--text)]'}`}>
            <GraduationCap size={16} strokeWidth={1.6} />
            <span className="leading-none">使用引导</span>
          </button>
          <button onClick={onSettings} title="设置"
            className="w-3/4 ml-auto h-9 flex items-center gap-3 px-3 rounded-md text-sm font-medium text-dim transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text)]">
