...
{/* 右侧拖拽手柄 */}
          ...
          <div style={{ width: rightPanelWidth, minWidth: 260 }} className="h-full flex-shrink-0 relative panel rounded-3xl overflow-hidden">
            <RightPanel messageCount={currentMessages.filter(m => m.role === 'assistant').length} projectId={currentProjectId} sideDialogueId={secondDialogueIdRef.current} dialogueId={currentDialogueId} onCollapse={() => setRightCollapsed(true)} />
          </div>
        </>
      )}
      </>) : (
