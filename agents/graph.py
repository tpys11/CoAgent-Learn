...
def dispatch_node(state: AgentState) -> AgentState:
    ...
    if state.get("profile"):
        context += f"学情诊断结果: {json.dumps(state['profile'], ensure_ascii=False)}\n"
    if state.get("knowledge"):
        context += f"知识库结果数: {len(state['knowledge'])}\n"
    if state.get("search_results"):
        context += f"搜索结果数: {len(state['search_results'])}\n"
...
def generate_node(state: AgentState) -> AgentState:
    ...
    if state.get("profile"):
        context += f"学情: {json.dumps(state['profile'], ensure_ascii=False)}\n"
    if state.get("knowledge"):
        context += f"知识库: {json.dumps(state['knowledge'], ensure_ascii=False)}\n"
    try:
        result = llm.chat_with_json(
...
def review_node(state: AgentState) -> AgentState:
    ...
    try:
        result = llm.chat_with_json(
            [{"role": "system", "content": REVIEW_PROMPT},
             {"role": "user", "content": state.get("generated", "")}],
    ...
    state.setdefault("steps", []).append(
        {"agent": "审核裁判", "status": "done", "detail": f"score={state['reviewed'].get('score', 0)}"})
    return state


def output_node(state: AgentState) -> AgentState:
    state.setdefault("steps", []).append({"agent": "信息整理与生成", "status": "微调输出"})
...
def route_dispatch(state: AgentState) -> str:
    result = state.get("_dispatch_result", {})
    if result.get("action") == "call_agent":
        agent = result.get("agent", "kb")
        return {"diagnose": "diagnose", "kb": "kb", "search": "search", "memory": "memory"}.get(agent, "enough")
    return "enough"


def route_review(state: AgentState) -> str:
    reviewed = state.get("reviewed", {})
    if reviewed.get("passed", True):
        return "passed"
...
def build_workflow() -> StateGraph:
    ...
    graph.add_conditional_edges("dispatch", route_dispatch, {
        "diagnose": "diagnose", "kb": "kb", "search": "search", "memory": "memory", "enough": "generate"
    })
    for node in ["diagnose", "kb", "search", "memory"]:
